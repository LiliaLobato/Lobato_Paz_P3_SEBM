...
volatile uint8_t g_read_data[30];
...
void RAM_CLEAR(uint8_t low_add) {
	...
	uint8_t i;
	for (i = 0; i < 30; i++) {
		I2C_write_byte(I2C_0, '\0'); //Escribe secuencia de inicializacion
		I2C_wait(I2C_0); //Checking if the I2C module is busy by cheking the busy flag
		I2C_get_ack(I2C_0); //Waiting for acknowledge
...
void RAM_CLEAR_ALL() {
	RAM_CLEAR(RAM_C1_LOW_ADDRESS);
	RAM_CLEAR(RAM_C2_LOW_ADDRESS);
	RAM_CLEAR(RAM_C3_LOW_ADDRESS);
	RAM_CLEAR(RAM_C4_LOW_ADDRESS);
	RAM_CLEAR(RAM_C5_LOW_ADDRESS);
}
...
volatile uint8_t RAM_read(uint8_t low_add) {
	...
	volatile uint8_t cadena; //Variable donde se almacenará la lectura
...
volatile BooleanType RAM_error_cmp(void) {
	uint8_t data[1] = "X";
	RAM_write(ERROR, &data);

	data[0] = RAM_read(RAM_ERROR_CMP_ADDRESS); //Guardamos un valor conocido en una direccion conocida, luego lo leemos para comprobar si hay algun error

	if ('X' == data[0]) //Luego lo comparamos con nuestro valor conocido
			{
		return TRUE;
	}
